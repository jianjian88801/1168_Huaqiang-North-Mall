        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('shangpinCollection','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('shangpinCollection','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('shangpinCollection','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shangpinCollection','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/huaqiangbeiershoushojiushangcheng/upload/shangpinCollectionMuBan.xls"
                        >批量导入商品收藏数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shangpinCollection','导入导出')"
                                style="display: inline-block"
                                action="huaqiangbeiershoushojiushangcheng/file/upload"
                                :on-success="shangpinCollectionUploadSuccess"
                                :on-error="shangpinCollectionUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shangpinCollection','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入商品收藏数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shangpinCollection','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shangpinCollection.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('shangpinCollection','查看')"
                          :data="dataList"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('shangpinCollection','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shangpinCollection','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shangpinCollection','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['shangpinCollectionDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "shangpinCollection/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            shangpinCollectionUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shangpinCollection/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            shangpinCollectionUploadError(data){
